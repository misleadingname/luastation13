function love.conf(t)
    ...
    t.trackpadtouch = false                                -- ? (changelist only change, not in wiki yet)

    t.audio.mic = false                                    -- Request and use microphone capabilities in Android (boolean)
    t.audio.mixwithsystem = true                           -- Keep background music playing when opening LOVE (boolean, iOS and Android only)

    t.window.title = "Lua Station 13"                      -- The window title (string)
    t.window.icon = "/resources/textures/core/icon.png"    -- Filepath to an image to use as the window's icon (string)
    ...
    t.window.fullscreentype =                              -- (blame the formatter for the new line)
    ...
    t.window.depth = false                                 -- ? (changelist only change, not in wiki yet)
    ...
    t.graphics.renderers = { "vulkan", "opengl", "metal" } -- ? (changelist only change, not in wiki yet)
    t.graphics.excluderenderers = {}                       -- ? (changelist only change, not in wiki yet)
    t.graphics.lowpower = false                            -- ? (changelist only change, not in wiki yet)
    ...
    t.modules.sensor = false                               -- ? (changelist only change, not in wiki yet)
